    ...
    A: std::fmt::Debug + Send + 'static,
{
    ...
    fn gen_waiter<'a>(
        &'a self,
        i: usize,
    ) -> impl Future<Output = Result<(Q, usize), ()>> + 'a + Send {
    ...
    fn gen_waiters<'a>(
        &'a self,
    ) -> HashMap<usize, Pin<Box<dyn Future<Output = Result<(Q, usize), ()>> + Send + 'a>>> {
        // all thoses allocations probably have a big overhead but are only
        // executed once per load
        let mut v: HashMap<usize, Pin<Box<dyn Future<Output = Result<(Q, usize), ()>> + Send>>> =
            HashMap::with_capacity(self.clients.len());
    ...
    E: std::fmt::Debug + From<tokio::sync::mpsc::error::SendError<Q>>,
{
    fn gen_future<R>(
        &mut self,
        query: Q,
        f: &'static (dyn Fn(Option<A>) -> R + Send + Sync + 'static),
        ...
        let (sender, receiver) = unsafe {
            (
                (Arc::get_mut_unchecked(&mut self.sender) as *mut Sender<Q>)
                    .as_mut()
                    .unwrap(),
                (Arc::get_mut_unchecked(&mut self.receiver) as *mut Receiver<A>)
                    .as_mut()
                    .unwrap(),
            )
        };
...
impl<Q, A, R, E> TaskClientService<Q, A, R, E> {
    pub fn new(
        client: TaskClient<Q, A, E>,
        fun: &'static (dyn Fn(Option<A>) -> Result<R, E> + Send + Sync + 'static),
    ) -> Self {
        TaskClientService { task: client, fun }
    }
}
